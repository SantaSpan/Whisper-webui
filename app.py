...
import demucs_scripts
from src.config import ApplicationConfig, VadInitialPromptMode
from src.download import (ExceededMaximumDuration, _perform_demucs_download,
                          download_url)
...
from src.vad import (AbstractTranscription, NonSpeechStrategy,
                     PeriodicTranscriptionConfig, TranscriptionConfig,
                     VadPeriodicTranscription, VadSileroTranscription)
...
class WhisperTranscriber:
    ...
    def transcribe_webui_simple(self, path, modelName, languageName, urlData, multipleFiles, microphoneData, task, vad, vadMergeWindow, vadMaxMergeSize, vadPadding, vadPromptWindow, 
        ...
        return self.transcribe_webui(path, modelName, languageName, urlData, multipleFiles, microphoneData, task, vadOptions, progress=progress)
    ...
    def transcribe_webui(self, path, modelName, languageName, urlData, multipleFiles, microphoneData, task, 
                         vadOptions: VadOptions, progress: gr.Progress = None, **decodeOptions: dict):
        try:
            sources = self.__get_source(path, urlData, multipleFiles, microphoneData)
    ...
    def __get_source(self, path, urlData, multipleFiles, microphoneData):
        return get_audio_source_collection(path, urlData, multipleFiles, microphoneData, self.inputAudioMaxDuration)
...
class DemucsController:
    ...
    def download_yt(self, url, destinationDirectory):
        return _perform_demucs_download(url, destinationDirectory=destinationDirectory)
...
def create_ui(app_config: ApplicationConfig):
    ...
    demucs = DemucsController(app_config=app_config)
    ...
    video_input = lambda : (gr.Text(label="URL (YouTube, etc.)"))
    
    demucs_inputs = lambda : [
        gr.Text(label="URL (YouTube, etc.)")
    ]

    demucs_ui = gr.Interface(fn=demucs.extract_voice, inputs=demucs_inputs(), outputs=[
        gr.Text(label="path")
    ])
    ...
    demo = gr.TabbedInterface([demucs_ui, simple_transcribe, full_transcribe], tab_names=["Demucs", "Simple", "Full"])
